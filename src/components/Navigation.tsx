...
const Navigation: FC<NavigationProps> = ({ variant = 'default' }) => {
  ...
  const captureMenuToggle = (nextState: boolean) => {
    if (typeof window === 'undefined') {
      return;
    }
    window.phCapture?.('nav_menu_toggle', { state: nextState ? 'open' : 'close' });
  };

  const handleToggleMenu = () => {
    setIsMenuOpen((prev) => {
      const nextState = !prev;
      captureMenuToggle(nextState);
      return nextState;
    });
  };
      ...
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5 lg:px-8">
        ...
        <div className="hidden items-center gap-8 text-[0.7rem] uppercase tracking-[0.35em] text-bone/60 md:flex">
          <a
            ...
            data-ph-label="tours"
            data-ph-location="header_desktop"
          >
            Toers
          </a>
          <a
      ...
      >
        <div className="flex flex-col gap-4 text-[0.7rem] uppercase tracking-[0.35em] text-bone/70">
          <a
            ...
            data-ph-event="nav_link_click"
            data-ph-label="tours"
            data-ph-location="header_mobile"
          >
            Toers
          </a>
          <a
